Model output files are in /data/ and follow the naming convention: ‘compiled_run_{flux}_a_b_v6_varz.pkl’ with flux:
‘CO2’: Gross CO2 captured (tCO2/y)
‘grem’: Mining/Grinding Emissions (tCO2/y)
‘grco’: Mining/Grinding Costs ($/y)
‘trem’: Transport Emissions (tCO2/y)
‘trco’: Transport Costs ($/y)
‘spem’: Spreading Emissions (tCO2/y)
‘spco’: Spreading Costs ($/y)
‘rock’: Rock applied (t rock/y)
‘pH’: Soil pH (top 10 cm)
‘P_mass’: Phosphorus Release (kg/ha/y)
‘K_mass’: Potassium Release (kg/ha/y)
‘feem_P’: Offset Phosphorus Emissions (tCO2/y)
‘feem_K’: Offset Potassium Emissions (tCO2/y)
‘feco_P’: Offset Phosphorus Costs ($/y)
‘feco_K’: Offset Potassium Costs ($/y)
Negative fluxes denote carbon capture/costs saved.

Each pickle file (*.pkl) contains a Python dictionary. The dictionary’s keys are unique points id. The value of the dictionary is a dictionary with keys and values:
...
A Python file is also provided (‘paper_data.py’) in /src/ which can be used to generate netCDF files of the fluxes. It contains the class PaperData with instance variables: scen_id (scenario id), unc_id (uncertainty id) and flx_name (flux name). The user can input different values of the instanced variables to produce netCDF files with the flux for the particular case. The netCDF file will have dimensions [768, 1152, 51] with 51 the years of the run 2020-2070. For more info, see ‘aa_paper_data.py’.
